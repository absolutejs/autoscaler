...
export const createPolicy = (policy: AutoscalerPolicy): AutoscalerPolicy => {
	if (!Number.isInteger(policy.min) || policy.min < 0) {
		throw new Error(`autoscaler: min must be a non-negative integer (got ${policy.min})`);
	}
...
export type CombineStrategy =
	| 'max'
	| 'avg'
	| ((readings: Array<{ signal: string; score: number; weight: number }>) => number);
	...
	const evaluate = async (): Promise<Decision> => {
		...
		const usable = readings
			...
			.map((r) => ({
				...
				signal: r.signal,
				weight:
					options.signals.find((s) => s.name === r.signal)?.weight ?? 1
			}));
